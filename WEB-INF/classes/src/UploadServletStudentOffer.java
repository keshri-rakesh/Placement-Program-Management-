...
@WebServlet(name = "UploadServletStudentOffer", urlPatterns = {"/UploadServletStudentOffer"})
@MultipartConfig(fileSizeThreshold = 1024 * 1024 * 100, // 100 MB
        maxFileSize = 1024 * 1024 * 1000, // 1 GB
        maxRequestSize = 1024 * 1024 * 1000)   	// 1 GB
public class UploadServletStudentOffer extends HttpServlet {
    ...
    protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
        ...
        try {
            ...
            try {
                ...
                String sql = "insert into uploadoffer(filename,path,add_date) values(?,?,?)";
                ps = con.prepareStatement(sql);
                //ps.setString(1, firstName);
                //ps.setString(2, lastName);
                ps.setString(1, fileName);
                ps.setString(2, path);
                ps.setTimestamp(3, add_date);
                int status = ps.executeUpdate();
                if (status > 0) {
                    session.setAttribute("fileName", fileName);
                    String msg = "" + fileName + "\n File uploaded successfully...";
                    request.setAttribute("msg", msg);
                    RequestDispatcher rd = request.getRequestDispatcher("/UploadOfferStudent.jsp");
