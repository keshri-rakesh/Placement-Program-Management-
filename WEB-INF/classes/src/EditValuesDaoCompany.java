...
public class EditValuesDaoCompany {
	...
	public void edit_user(CompanyBean obj_User_Bean){
		...
		try {
            con = DBConnection.createConnection();
			String querryy="update company set company_name=?,language=?,job_type=?,location=?,status=?,upcoming_date=? where company_id=?";
			ps=con.prepareStatement(querryy);		
			ps.setString(1, obj_User_Bean.getcompany_name());
			ps.setString(2, obj_User_Bean.getlanguage());
			ps.setString(3, obj_User_Bean.getjob_type());
			ps.setString(4, obj_User_Bean.getlocation());
			ps.setString(5, obj_User_Bean.getstatus());
			ps.setString(6, obj_User_Bean.getupcoming_date());
			ps.setString(7, obj_User_Bean.getcompany_id());
			ps.executeUpdate();	
		} catch (Exception e) {
				System.out.println(e);
		}	
